...
let number_of_research_input_boxes;
function research_input_change(event) {
    number_of_research_input_boxes = event.target.value;
    var inputs = `<div id="research_input_group">`
    $('#research_input_group').remove();
    for (let index = 0; index < number_of_research_input_boxes; index++) {
        inputs += 
            `<div class="row m-2 p-3" id="research_input_group_style">
                <div class="col-md-12"><h5>Research Publication- `+(index+1)+` </h5></div>
                    <div class="col-md-6">
                        <label for="research_title_`+(index+1)+`" class="form-label">Paper Title</label>
                        <input type="text" class="form-control" id="research_title_`+(index+1)+`">
                    </div>

                    <div class="col-md-6 mt-2">
                        <label for="paper_link_`+(index+1)+`" class="form-label">Paper Link</label>
                        <input class="form-control" id="paper_link_`+(index+1)+`" />
                    </div>

                    <div class="col-md-6 mt-2">
                        <label for="research_description_`+(index+1)+`" class="form-label">Add a Description (1-2 lines max)</label>
                        <textarea class="form-control" id="research_description_`+(index+1)+`"></textarea>
                    </div>
            </div>`
    }
    $('#research_inputs').append(inputs);
}
...
function printWithBio(event) {
    event.preventDefault(); 
    // ------ OG Code ------
    let firstName = document.getElementById("firstName").value.trim();
    let lastName = document.getElementById("lastName").value.trim();
    let email = document.getElementById("email").value.trim();
    let currlocation = document.getElementById("currlocation").value.trim();
    ...
    doc.setFontSize(11)
    doc.text(currlocation , 10, currYPosition);
    doc.setFontSize(20)
    ...
    doc.setFontSize(14)
    doc.setFont("Times", "bold", "normal")
    doc.text("About Me : ", 10, currYPosition);
    ...
    for (let index = 0; index < number_of_project_input_boxes; index++) {
        ...
        X += doc.getStringUnitWidth(document.getElementById("title_"+(index+1)).value.trim() + `   |   Link : `) * 4 - 3
        // X += 50
        // X = 110
        doc.setTextColor(0, 0, 255);
...
function printWithResearch(event) {
    event.preventDefault(); 
    // ------ OG Code ------
    let firstName = document.getElementById("firstName").value.trim();
    let lastName = document.getElementById("lastName").value.trim();
    ...
    doc.setFontSize(14)
    doc.setFont("Times", "bold", "normal")
    doc.text("Research Publications : ", 10, currYPosition);
    doc.line(10, currYPosition+1, 200, currYPosition+1, 'S') 
    doc.setFont("Times", "normal", "normal")
    doc.setFontSize(11)
    currYPosition += 5
    for (let index = 0; index < number_of_research_input_boxes; index++) {
        doc.setFont("Times", "bold", "normal")
        console.log(document.getElementById("research_title_"+(index+1)).value.trim())
        doc.text(document.getElementById("research_title_"+(index+1)).value.trim(), 15, currYPosition, {maxWidth: 185, align: "justify"});
        doc.setFont("Times", "normal", "normal")
        currYPosition += 4.5
        doc.text(document.getElementById("paper_link_"+(index+1)).value.trim() , 15, currYPosition, {maxWidth: 185, align: "justify"});
        currYPosition += 4.5
        var lineHeight = doc.getLineHeight(document.getElementById("research_description_"+(index+1)).value.trim()) / doc.internal.scaleFactor
        var splittedText = doc.splitTextToSize(document.getElementById("research_description_"+(index+1)).value.trim(), 185)
        var lines = splittedText.length  // splitted text is a string array
        doc.text(document.getElementById("research_description_"+(index+1)).value.trim(), 15, currYPosition, {maxWidth: 185, align: "justify"});
        var blockHeight = lines * lineHeight
        currYPosition += blockHeight
        currYPosition += 3
    }
